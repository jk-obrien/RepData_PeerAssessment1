...
On average, across all the days, the interval with the most steps is `r busiest` - `r after`, with `r scount` steps.
...
The mean and median are now the same. These values are also the same as the mean calculated before imputing missing values (`r daily_mean`) so, other than
